...
The worker runtime has separate supervised loops for claims, batched lease renewal, worker heartbeats, schedule materialization, notification listening, retention, and graceful shutdown. One worker scans queue demand per heartbeat interval and publishes the sample for every worker to report. A handler is an async Rust function registered under an explicit stable name and version.
