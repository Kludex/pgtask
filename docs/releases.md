...
Configure the GitHub `pypi` environment as a trusted publisher for the PyPI project. Add `CARGO_REGISTRY_TOKEN` as a repository secret. Allow GitHub Actions to write packages so it can publish to `ghcr.io`.
...
The release workflow rejects a tag that differs from the Cargo workspace version. It reruns the full Rust suite on PostgreSQL 17 and 18 before any artifact is published.
...
The final GitHub release is created only after PyPI, crates.io, the signed image, and the signed chart succeed.
