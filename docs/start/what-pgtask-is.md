...
The engine covers the work that usually pulls in a broker, a cron container, and a workflow service:
...
task available again, and your handler runs a second time. This is not a defect you can configure away. Any system that
survives a process dying at an arbitrary instant behaves this way.

So the rule is: **your handler must be safe to run again.** In practice that means pushing a stable key to whatever you
call:
...
!!! note "Idempotency keys work at two levels"
    ...
    *side effects*. They solve different problems, and you usually want both. See
...
A separate broker gives you throughput and takes away consistency.

The moment your queue lives outside your database, enqueueing becomes a second write that can succeed when your
transaction fails, or fail when it succeeds. You then spend real effort on outbox tables and reconciliation jobs to get
back what a single transaction gave you for free.
