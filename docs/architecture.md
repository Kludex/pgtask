...
Queues may set a hard outstanding-task capacity. New admission atomically increments an O(1) counter only on a
capacity-limited queue and rejects the insert with SQLSTATE `PT001` when full. Unlimited queues avoid this coordination
point. Configure a limit before starting producers so its initial count is stable. Scheduled catch-up materializes only
the available number of occurrences and leaves the rest due.
