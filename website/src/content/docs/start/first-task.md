...
description: Write a handler, run a worker, and enqueue work.
---
...
`render.request(...)` builds the request from the handler you already declared, so the payload type is checked against
the handler's signature instead of being a loose dictionary.
...
:::caution[The notification is not the task]
PostgreSQL sends the wake-up after your transaction commits. The task row is the source of truth; the notification only
saves the worker from polling. If a notification is ever lost, a reconciliation poll finds the task anyway.
:::
...
That path is worth understanding in detail before you put real work through it. Read
[How a task runs](/pgtask/architecture/task-lifecycle/).
